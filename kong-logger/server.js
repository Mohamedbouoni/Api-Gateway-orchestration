...
// ── Vault secret loader ───────────────────────────────────────────────────────
// Fetches PLATFORM_DB_URL from Vault KV v2 at startup using native Node http.
// Falls back to process.env.PLATFORM_DB_URL when Vault is disabled/unreachable.
function loadPlatformDbUrlFromVault() {
  return new Promise((resolve) => {
    const vaultEnabled = (process.env.VAULT_ENABLED || "").toLowerCase() === "true";
    if (!vaultEnabled) {
      resolve(process.env.PLATFORM_DB_URL || "");
      return;
    }

    const vaultAddr  = process.env.VAULT_ADDR  || "http://vault.ai-data.svc.cluster.local:8200";
    const vaultToken = process.env.VAULT_TOKEN || "dev-root-token";
    const vaultMount = process.env.VAULT_MOUNT || "secret";
    const secretPath = `${vaultMount}/data/platform/postgres`;
    ...
    http.get(url, reqOptions, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          const d    = body.data && body.data.data;
          if (!d || !d.user || !d.password || !d.host || !d.port || !d.db) {
            throw new Error("Unexpected Vault response shape");
          }
          const dbUrl = `postgresql://${d.user}:${d.password}@${d.host}:${d.port}/${d.db}`;
          console.log("[server] Loaded PLATFORM_DB_URL from Vault");
          resolve(dbUrl);
        } catch (err) {
          console.warn("[server] Vault parse error; falling back to env:", err.message);
          resolve(process.env.PLATFORM_DB_URL || "");
        }
      });
    }).on("error", (err) => {
      console.warn("[server] Vault unreachable; falling back to env:", err.message);
      resolve(process.env.PLATFORM_DB_URL || "");
    });
  });
}

// ── Main startup (async to allow Vault fetch before server starts) ────────────
async function main() {
  const PLATFORM_DB_URL = await loadPlatformDbUrlFromVault();
